    ...
    startEditing: function (attr, latlngs) {
        ...
        if (this.hasEditSylte) {
            ...
            for (var idx = 0; idx < config.style.length; idx++) {
                ...
                var attrVal = attr.style[attrName] || edit.defval;
            ...
            //注记属性 
            if (attr.style.label) {
                var configLbl = thisWidget.attrConfig["label"] || {};
                var defStyleLbl = thisWidget.getDefStyle("label")//赋值默认样式

                var parname = "plot_attr_stylelabel_";
                var inHtml = '';
                for (var idx = 0; idx < configLbl.style.length; idx++) {
                    var edit = configLbl.style[idx];
                    if (edit.type == "hidden") continue;

                    var attrName = edit.name;
                    var attrVal = attr.style.label[attrName] || defStyleLbl[attrName];
        ...
        for (var idx = 0; idx < attrcfg.length; idx++) {
            ...
            var attrName = edit.name;
            var attrVal = attr.attr[attrName] || edit.defval || "";
